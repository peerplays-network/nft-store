    ...
    stripe.charges.create(chargePayload, (err, charge) => {
        ...
        db.orders.insertOne(orderDoc, (err, newDoc) => {
            ...
            .then(() => {
                ...
                if(charge.paid === true){
                    new PeerplaysService().sendOperations({
                        operations: [{
                            op_name: 'transfer',
                            fee_asset: config.peerplaysAssetID,
                            from: adminUser.peerplaysAccountId,
                            to: req.session.peerplaysAccountId,
                            amount: {
                                amount: req.session.cart.ppyAmount * Math.pow(10, config.peerplaysAssetPrecision),
                                asset_id: config.peerplaysAssetID
                            }
                        }]
                    }, adminUser.peerIDAccessToken).then((result) => {
                        if(result.status === 200){
                            ...
                            req.session.message = 'Your payment was successfully completed but there was some error while transferring the funds to your account. Please contact us.';
                            req.session.paymentEmailAddr = newDoc.ops[0].orderEmail;
                            req.session.paymentApproved = true;
                            req.session.paymentDetails = `<p><strong>Order ID: </strong>${newId}</p><p><strong>Transaction ID: </strong>${charge.id}</p>`;
